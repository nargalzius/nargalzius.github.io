...
var preview = {
	...
	init: function(obj) {
		...
		if(this.img || this.swf)
		{
			...
			{
				if(this.swf.page)
					this.loadBG(this.swf.page);
				else
					this.initFlash(this.swf);
			}
			else
		...
		{
			trace('no initialization variables!');
			$('<div>').html(errorMSG).attr('id', '#errorMSG').appendTo(this.content);
			$('#preloader').remove();
		}
	},
	initImage: function() {
